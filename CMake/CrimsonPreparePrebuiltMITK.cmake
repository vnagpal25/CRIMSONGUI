...
function(_crimson_try_fix_ctk_imported_location path out_var)
  ...
  # Dedupe duplicated CTK-build segment first; then map bin/<Config>/ -> lib/<Config>/ for MSVC import libs.
  ...
  if(path MATCHES "\\.lib$")
    string(REPLACE "/bin/" "/lib/" _lib2 "${path}")
    string(REPLACE "\\bin\\" "\\lib\\" _lib2 "${_lib2}")
    if(EXISTS "${_lib2}")
      set(${out_var} "${_lib2}" PARENT_SCOPE)
      return()
    endif()
  endif()
  # If layout is .../CTK-build/bin/<Config>/foo.lib, try .../CTK-build/lib/<Config>/foo.lib (dedupe may still leave a bad bin path).
  if(path MATCHES "\\.lib$")
    get_filename_component(_leaf "${path}" NAME)
    get_filename_component(_cfgdir "${path}" DIRECTORY)
